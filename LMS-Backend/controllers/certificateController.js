...
exports.getCertificateData = async (req, res) => {
    try {
        ...
        if (!certificateId) {
            certificateId = `CERT-${String(courseId).slice(-6)}-${String(userId).slice(-6)}-${Date.now().toString().slice(-6)}`;
        ...
        const durationHours = totalDuration > 0
        ...
        res.json({
            ...
            certificate: {
                ...
                instructorName: course.instructor ? course.instructor.name : 'Traincape Instructor',
...
exports.downloadCertificate = async (req, res) => {
    ...
    try {
        ...
        const instructorName = course.instructor ? course.instructor.name : 'Traincape Instructor';
        ...
        // Send PDF
        const filename = `Certificate_${courseName?.replace(/[^a-z0-9]/gi, '_') || 'Course'}.pdf`;
