...
const EditProfileModal = ({ user, onSave, onCancel }) => {
  ...
  const handlePhotoChange = async (e) => {
    ...
    // Optional: client-side validatiion (size, type)
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // Update user in local storage immediately if you want, or wait for save
        // But the backend is already updated for the photo
        const currentUser = JSON.parse(localStorage.getItem("user") || "{}");
        const updatedUser = { ...currentUser, photoUrl: res.data.url };
    ...
    >
      ...
      >
        ...
        <div className="w-full p-5 flex items-center justify-center flex-col">
          <div className="w-24 h-24 rounded-full overflow-hidden mb-4 flex items-center justify-center relative">
            {imgLoading ? (
              ...
              <div className="bg-PrimaryDark rounded-full w-full h-full flex items-center justify-center">
                <span className="text-2xl font-semibold text-white">
                  {(firstName[0] || "").toUpperCase()}
                </span>
              </div>
            )}
            <div className="hover:opacity-100  opacity-0 transition-all duration-500 ease-in-out w-full h-full absolute top-0 left-0 z-50  bg-gray-600/50 flex items-center justify-center cursor-pointer">
              <button
                type="button"
                title="Edit photo"
        ...
        {success && (
          <div style={{ color: "green", marginBottom: 10, fontWeight: 600 }}>
            {success}
          </div>
        )}
        ...
        <div className="flex items-center justify-between w-full gap-5">
          ...
          <Button type="submit" disabled={loading} className="w-1/2 bg-Accent">
            {loading ? "Saving..." : "Save"}
          </Button>
        </div>
      </form>
