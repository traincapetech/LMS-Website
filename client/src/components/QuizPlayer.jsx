...
const QuizPlayer = ({ quiz, courseId, onComplete, isCompleted }) => {
    ...
    try {
      if (courseId) {
        await progressAPI.markQuizComplete({
          courseId,
          quizId: quiz._id || quiz.quizId || quiz.id,
          itemId: quiz.itemId || quiz._id || quiz.id,
  ...
  if (showResult) {
    // If showing result from "Already Completed" state (not just submitted)
    if (isCompleted && !isSubmitted) {
      return (
        ...
        >
          ...
          <p
            style={{ fontSize: "18px", color: "#a1a1aa", marginBottom: "30px" }}
          >
          ...
          {currentQuestion.answers.map((ans) => {
            ...
            return (
              <div
                className={`border-2 ${
                  selected
                    ? " border-blue-600 bg-[rgba(126, 34, 206, 0.1)]"
                    : "border-[#333] bg-[272732]"
                }`}
                key={ans.id}
                onClick={() =>
                  handleOptionClick(
